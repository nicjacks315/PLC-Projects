...
There's some glaring issues with the control systems designed here, but it all works, the fish and plants are happy, and that's all that really matters in the end.
...
## Serialization
A lack of supplementary serial pins (TX/RX pins are pulled up directly from the USB-A port) on the UNO R3 means that functionality needs to be mimicked with software on digital pins that support interrupts. The following libraries are used interchangably through different versions, but ultimately `SoftwareSerial` was chosen for it's consistency. Alternatives provided better performance at the expense of occasional undefined behavior or faulty pin readings.
...
The following diagram still represents accurate wiring for the constructed system, but MCU pinning has changed slightly. All relays pictured below are installed in the electrical box, but their controlled effectors/actuators may not be installed. For example: the ACU was deemed unnecessary for testing in the temperate climate of Spring in Northern California.
